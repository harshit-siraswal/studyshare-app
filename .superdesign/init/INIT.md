...
- These outputs intentionally include full source code. Before generation/export, scrub or redact API keys, tokens, credentials, proprietary algorithms, and any sensitive internal logic that should not be shared.
- Never commit generated `.superdesign/` analysis files. Add `.superdesign/` to `.gitignore` and confirm it is excluded from version control before saving outputs.
- Review code/comments for PII or test data and remove/redact it before writing files.
- Mark generated files as internal-use-only documentation and restrict distribution to authorized collaborators.
...
**IMPORTANT**: Include FULL source code for each component. SuperDesign needs implementation, not summaries.
...
- Source code in fenced code blocks
...
2. Trace ALL local imports (relative `./Foo`, `../Bar`, alias `@/components/Baz`) while skipping third-party/build directories (`node_modules`, `.next`, `dist`, `build`, `.nuxt`, `out`)
3. Maintain a visited set during traversal; if a file repeats, annotate it as `[cycle]` and stop traversing that branch
4. Enforce recursion boundaries: configurable max depth (default `10`) and configurable global node cap (default `500`)
5. If max depth or node cap is reached, stop that branch and annotate `[truncated]`
6. Present as an indented tree showing every resolved dependency file
