...
Future<void> animateThemeTransition(
  ...
  final pixelRatio = _getDevicePixelRatio(context);
  ui.Image? image;
  try {
...
class _SunMoonTransitionState extends State<_SunMoonTransition>
  ...
  Widget _buildStars(Size size, double t) {
    ...
    return Positioned.fill(
      child: Opacity(
        opacity: starOpacity,
        child: CustomPaint(painter: _StarsPainter(opacity: starOpacity)),
      ),
    );
  }
...
class _StarsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final rng = math.Random(
      42,
    ); // Deterministic seed for consistent star positions
    final paint = Paint()
    ...
    for (int i = 0; i < 60; i++) {
      ...
      final y =
          rng.nextDouble() * size.height * 0.6; // Stars mostly in upper area
      ...
      paint.color = Colors.white.withValues(
        alpha: opacity * 0.4 * twinkle + opacity * 0.3,
      );
