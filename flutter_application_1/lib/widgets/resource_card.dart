...
class ResourceCardState extends State<ResourceCard> {
  ...
  Widget _buildModerationCard(BuildContext context, bool isDark) {
    final title = widget.resource.title.trim().isEmpty
        ? 'Untitled resource'
        : widget.resource.title.trim();
    final status = Resource.normalizeStatusValue(widget.resource.status);
    final subject = _normalizedMetaValue(widget.resource.subject);
    final semester = _normalizedMetaValue(widget.resource.semester);
    final branch = _normalizedMetaValue(
      getBranchShortLabel(widget.resource.branch),
    );
    final metaParts = <String>[?subject, ?semester, ?branch];
    final metaText = metaParts.join(' | ');
    final timeLabel = _formatTimeAgo(widget.resource.createdAt);
    final uploaderName =
        _normalizedMetaValue(widget.resource.uploadedByName) ??
        _normalizedMetaValue(widget.resource.uploadedByEmail)?.split('@').first;
    final isOwner =
        widget.userEmail.trim().isNotEmpty &&
        widget.resource.uploadedByEmail.trim().isNotEmpty &&
        widget.userEmail.trim().toLowerCase() ==
            widget.resource.uploadedByEmail.trim().toLowerCase();

    Color badgeColor;
    String badgeText;
    switch (status) {
      case Resource.approvedStatus:
        badgeColor = const Color(0xFF22C55E);
        badgeText = 'Approved';
        break;
      case Resource.rejectedStatus:
        badgeColor = const Color(0xFFEF4444);
        badgeText = 'Rejected';
        break;
      case Resource.pendingStatus:
      default:
        ...
        child: ElevatedButton.icon(
          ...
          style: ElevatedButton.styleFrom(
            ...
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(8),
              side: outlined ? BorderSide(color: background) : BorderSide.none,
            ),
            elevation: 0,
    ...
    if (status == Resource.pendingStatus) {
    ...
    if (status == Resource.approvedStatus && widget.onRetract != null) {
      actionButtons.add(
        buildActionButton(
          label: 'Retract',
    ...
    final shouldShowDelete =
        ...
        (isOwner || status != Resource.pendingStatus);
    if (shouldShowDelete) {
      actionButtons.add(
        buildActionButton(
          label: 'Delete',
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(12),
            border: Border.all(
              color: isDark ? const Color(0xFF2D3036) : Colors.grey.shade200,
            ),
            boxShadow: [
              BoxShadow(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (uploaderName != null) ...[
                          ...
                          Text(
                            'Uploaded by $uploaderName - $timeLabel',
                            style: GoogleFonts.inter(
                              fontSize: 11,
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showModerationControls) {
      return _buildModerationCard(context, isDark);
    }
