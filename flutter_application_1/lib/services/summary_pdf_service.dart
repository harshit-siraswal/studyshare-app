...
class SummaryPdfService {
  ...
  Uint8List _buildPdfBytes({required String title, required String summary}) {
  ...
  List<String> _buildWrappedLines({
    ...
    final output = <String>[
      '[H]$title',
      '',
      '[H]Generated by MyStudySpace AI',
      '',
    ];
    ...
    for (var raw in rawLines) {
      ...
      final isHeading = _looksLikeHeadingLine(collapsed);
      if (isHeading) {
        output.add('[H]${collapsed.replaceAll(RegExp(r'[:\-\s]+$'), '')}');
        output.add('');
        continue;
      }

      String cleaned = collapsed;
  ...
  bool _looksLikeHeadingLine(String line) {
    ...
    final words = plain
        ...
        .length;
    if (words <= 7 && RegExp(r'^[A-Z]').hasMatch(plain)) return true;
    return false;
  }
  ...
  String _buildPageStream(List<String> lines) {
    ...
    final watermark = _escapePdfText('studyshare');

    stream.writeln('q');
    stream.writeln('0.93 g');
    stream.writeln('BT');
    stream.writeln('/F2 42 Tf');
    stream.writeln('1 0 0 1 160 410 Tm');
    ...
    for (final raw in lines) {
      ...
      stream.writeln(isHeading ? '/F2 14 Tf' : '/F1 12 Tf');
